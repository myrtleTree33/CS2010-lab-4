...
public class BinarySearchTree<E extends Comparable<E>> extends BinaryTree<E>
   ...
   * when a value is deleted that has two children the inorder sucessor replaces
   * it in the tree.
   * 
   * @post The object is not in the tree.
   * @param target
  ...
  /**
   * Return the contents of the BinarySearchTree as a List of items in ascending
   * order. (Note the exercise suggests returning a string of item separated by
   * newline characters, but a List is more general and useful for the testing
   * performed by the exercise. Also, the toString method of the BinaryTree
   * class would be hidden and it is useful to verify some of the tests.
   */
    ...
    if (compResult < 0) {
      ...
      if (localRoot.left == null) {
        ...
        if (localRoot.right.left == null) {
          ...
          // Search for the inorder predecessor (ip) and
          // replace deleted node's data with ip.
          localRoot.data = findLargestChild(localRoot.right);
          return localRoot;
        }
      }
    }
  ...
  /**
   * Find the node that is the inorder sucessor and replace it with its right
   * child (if any).
   * 
   * @post The inorder sucessor is removed from the tree.
   * @param parent
   *          The parent of possible inorder sucessor (is)
